...
function fromPostgres(error) {
	...
	if (message.includes("PGRST202") || error.code === "PGRST202" || error.code === "42883") return new GameError("DB_OUT_OF_DATE", "The database is missing a function this version of the game needs. Run the pending migrations.", 500);
	if (message.includes("PGRST203") || error.code === "PGRST203") return new GameError("DB_AMBIGUOUS", "The database has two versions of the same function. An older migration left one behind; re-run the latest migrations.", 500);
	if (error.code === "42501" || message.includes("permission denied")) return new GameError("DB_FORBIDDEN", "The game server is not permitted to write to this table. A grant is missing; re-run the latest migrations.", 500);
	return null;
}
//#endregion
...
async function placeBid(store, actor, gameId, quantity, face) {
	...
	const verdict = checkBid(state, {
		quantity,
		face
	});
	if (!verdict.legal) throw new GameError("ILLEGAL_BID", verdict.detail);
	const burst = isBurst(round.turn_player_id, actor.id);
	if (burst && opensFarewell(state)) throw new GameError("FAREWELL_OPENING", "This round opens with their bid. You can cut in once the face is set.", 409);
